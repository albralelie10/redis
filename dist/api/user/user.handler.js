...
const getAllUsers = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const key = "get-users";
        const cacheValue = yield redis_1.default.get(key);
        if (cacheValue) {
            const freshUsers = JSON.parse(cacheValue);
            console.log("CACHE HIT");
            return res.json(freshUsers);
        }
        console.log("CACHE MISS");
        const response = yield user_model_1.Users.find();
        const users = yield response.toArray();
        // Convertir el array de usuarios a una cadena JSON antes de almacenarlo en Redis
        const usersJson = JSON.stringify(users);
        // Almacenar en Redis
        yield redis_1.default.set(key, usersJson, "EX", 60);
